...
export const ActiveTicketCard: React.FC<ActiveTicketCardProps> = ({
  ...
  const getWhenToLeave = useCallback(() => {
    ...
    return { urgent: false, message: `Vous devriez partir dans ~${leaveIn} min` };
  ...
  const handleConfirmPresence = useCallback(async () => {
    try {
      await axiosClient.post(`/tickets/${activeTicket?.id}/en-route`, {
        estimated_travel_minutes: distanceInfo?.travelTimes?.[preferredTransportMode],
      });
    ...
    return (
      ...
      >
        <View style={styles.calledContent}>
          <Animated.View style={[styles.calledIcon, { transform: [{ scale: pulseAnim }] }]}>
          ...
          <Text style={styles.calledSubtext}>
        ...
        <View className='flex-wrap flex-col justify-between gap-2' style={styles.calledActions}>
          ...
          >
            ...
            <Text style={styles.confirmButtonText}>Je confirme ma présence</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Ionicons name={hasRecalled ? "checkmark-done" : "refresh"} size={20} color="white" />
  ...
  return (
    ...
    >
      ...
      <View style={styles.ticketSection}>
        ...
        <View style={styles.serviceInfo}>
          ...
          <Text style={styles.ticketTime}>
            Pris à {new Date(activeTicket.created_at || Date.now()).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.statsRow}>
        <View style={styles.statItem}>
          ...
          <Text style={styles.statValue}>
            <Text style={styles.statHighlight}>{position}ème</Text> / {queueLength}
          </Text>
        </View>
      ...
      {distanceInfo && (
        <View style={styles.distanceRow}>
          ...
          <View style={styles.distanceItem}>
            ...
            <Text style={styles.distanceValue}>{formatTravelTime(distanceInfo.travelTimes.motorcycle)}</Text>
          </View>
          <View style={styles.distanceItem}>
      ...
      {/* When to Leave Alert */}
      {whenToLeave && (
        <View style={[styles.leaveAlert, whenToLeave.urgent && styles.leaveAlertUrgent]}>
          <Ionicons
            name={whenToLeave.urgent ? "warning" : "time"}
            size={16}
            color={whenToLeave.urgent ? "#DC2626" : "#F59E0B"}
          />
          <Text style={[styles.leaveText, whenToLeave.urgent && styles.leaveTextUrgent]}>
            {whenToLeave.message}
          </Text>
        </View>
      )}
      ...
      <View style={styles.actionsRow}>
        ...
        >
          ...
          <Text style={styles.confirmPresenceText}>Je confirme ma présence</Text>
        </TouchableOpacity>
