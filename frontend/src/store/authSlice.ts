...
try {
  initialUser = persistedUser ? (JSON.parse(persistedUser) as User) : null;
} catch {
  initialUser = null;
...
export const registerEstablishment = createAsyncThunk(
  "auth/registerEstablishment",
  async (payload: {
    admin_name: string;
    admin_email: string;
    ...
    try {
      const { data } = await api.post(
        "/api/onboarding/register-establishment",
        payload,
      );
...
export const subscribe = createAsyncThunk(
  "auth/subscribe",
  async (payload: { plan: string; paid: boolean }) => {
    try {
...
const slice = createSlice({
  ...
  extraReducers: (b) => {
    ...
    b.addCase(login.rejected, (s, a: any) => {
      s.loading = false;
    ...
    b.addCase(signup.rejected, (s, a: any) => {
      s.loading = false;
      s.error = a?.error?.message || "Signup failed";
    });
    b.addCase(registerEstablishment.pending, (s) => {
      s.loading = true;
      s.error = undefined;
    });
    b.addCase(
      registerEstablishment.fulfilled,
      (
        s,
        a: PayloadAction<{ token: string; user: User; next_step: string }>,
      ) => {
        s.loading = false;
        s.user = a.payload.user;
        s.token = a.payload.token;
    ...
    b.addCase(registerEstablishment.rejected, (s, a: any) => {
      s.loading = false;
      s.error = a?.error?.message || "Registration failed";
    });

    b.addCase(subscribe.pending, (s) => {
      s.loading = true;
      s.error = undefined;
    });
    b.addCase(
    ...
    b.addCase(subscribe.rejected, (s, a: any) => {
      s.loading = false;
      s.error = a?.error?.message || "Subscription failed";
    });
    ...
    b.addCase(refreshMe.pending, (s) => {
      s.loading = true;
    });
    b.addCase(refreshMe.rejected, (s) => {
      s.loading = false;
    });
