...
const LinkItem = ({ to, icon: Icon, label, isCollapsed }: LinkItemProps) => {
  const linkContent = (
    <NavLink
      to={to}
    ...
    return (
      <TooltipProvider>
        <Tooltip delayDuration={0}>
          ...
          <TooltipContent side="right" className="bg-card border-border text-card-foreground px-3 py-2 text-sm font-medium">
            <p>{label}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
export default function Sidebar() {
  ...
  const navigationItems = [
    { to: '/', icon: LayoutDashboard, label: 'Tableau de bord', roles: ['all'] },
    { to: '/queues', icon: ListOrdered, label: 'Files d\'attente', roles: ['agent', 'admin'] },
    { to: '/queues/called', icon: Activity, label: 'Tickets appelés', roles: ['agent', 'admin'] },
    { to: '/queues/absent', icon: Ban, label: 'Absents', roles: ['agent', 'admin'] },
    { to: '/queues/priority', icon: BadgeAlert, label: 'Prioritaires', roles: ['agent', 'admin'] },
    { to: '/agents', icon: Users, label: 'Agents', roles: ['admin'] },
    { to: '/services', icon: Ticket, label: 'Services', roles: ['admin'] },
    { to: '/establishments', icon: Building2, label: 'Établissements', roles: ['admin'] },
    { to: '/stats', icon: BarChart, label: 'Statistiques', roles: ['admin'] },
    { to: '/settings', icon: SettingsIcon, label: 'Paramètres', roles: ['all'] },
  ]

  const filteredItems = navigationItems.filter(item => 
    item.roles.includes('all') || item.roles.includes(role as string)
  )
